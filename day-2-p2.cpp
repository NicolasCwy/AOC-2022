...
std::unordered_map<std::string, std::string> strength = {{"rock", "scissors"},
  {"paper", "rock"},
  {"scissors", "paper"}};
...
std::vector<std::pair<std::string, std::string>> getInput(std::string filename) {
  ...
  if (file.is_open())
  {
    while (getline(file, line))
    {
      ...
      std::pair<std::string, std::string> pair = std::make_pair(inputToSymbol.at(colOne), colTwo);
...
int main(int argc, char *argv[])
{
  ...
  for (std::vector<std::pair<std::string, std::string>>::iterator it = input.begin(); it != input.end(); ++it)
  {
    ...
    std::string outcome = std::get<1>(*it);
    std::string mySymbol = "";
    ...
    if (outcome == "Z") {
      score += 6;
      mySymbol = weakness.at(oppSymbol);
    // 2. beaten by opponent (col 1 beat col 2)
    } else if (outcome == "X") {
      mySymbol = strength.at(oppSymbol);
